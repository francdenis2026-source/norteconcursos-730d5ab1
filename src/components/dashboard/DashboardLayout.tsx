          ...
          <div className={cn("flex items-center gap-3", isCollapsed ? "justify-center" : "")}>
            ...
            {!isCollapsed && (
              <div className="flex flex-col">
                <span className="text-xs font-bold truncate">{user?.name || "Aluno"}</span>
                <span className="text-[10px] text-muted-foreground">
                  {user?.role || "Teste Gratuito"}
                </span>
              </div>
            )}
          </div>
        ...
        {!isSupabaseConfigured && (
          <div className="bg-amber-50 border-b border-amber-100 px-4 py-1 text-center">
            <p className="text-[10px] md:text-xs text-amber-800 font-medium">
              Ambiente demonstrativo — configure o Supabase para ativar dados reais.
            </p>
          </div>
        )}
