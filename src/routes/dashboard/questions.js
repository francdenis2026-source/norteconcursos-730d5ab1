...
import { Search, Filter, MapPin, GraduationCap, Calendar, Briefcase, Trophy, CheckCircle2 } from 'lucide-react';
...
function QuestionsCatalog() {
    ...
    const filteredContests = contests.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        c.agency.toLowerCase().includes(searchTerm.toLowerCase()) ||
        c.role.toLowerCase().includes(searchTerm.toLowerCase()));
    const handleSetFocus = (contest) => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input placeholder="Pesquisar por órgão, cargo ou banca..." className="pl-10" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredContests.map((contest) => (<Card key={contest.id} className={cn("flex flex-col border-2 transition-all hover:shadow-md", focusedContest?.id === contest.id ? "border-secondary" : "border-border")}>
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start gap-2">
                ...
                {focusedContest?.id === contest.id && (<div className="inline-flex items-center gap-1 text-[10px] font-bold text-secondary">
            ...
            <CardFooter className="pt-4 border-t gap-2">
              ...
              <Button className={cn("flex-1 text-xs", focusedContest?.id === contest.id ? "bg-muted text-muted-foreground" : "bg-primary")} disabled={focusedContest?.id === contest.id} onClick={() => handleSetFocus(contest)}>
                Definir Foco
              </Button>
            </CardFooter>
          </Card>))}
